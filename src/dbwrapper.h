...
public:
    ...
    template <typename K, typename V>
    bool Read(const K& key, V& value) const noexcept
    {
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        ssKey.reserve(ssKey.GetSerializeSize(key));
        ssKey << key;
    ...
    template <typename K, typename V>
    bool Write(const K& key, const V& value, bool fSync = false) noexcept
    {
    ...
    template <typename K>
    bool Erase(const K& key, bool fSync = false) noexcept
    {
        CDBBatch batch(&obfuscate_key);
        batch.Erase(key);
        return WriteBatch(batch, fSync);
    ...
    bool WriteBatch(CDBBatch& batch, bool fSync = false) noexcept;
